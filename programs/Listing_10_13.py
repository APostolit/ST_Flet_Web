# Router_6
"""Пример использования маршрутизатора — макет, навигация,
вложенные маршруты, параметры, загрузчики, авторизация."""
...
def home_loader(params):
    return {"greeting": "Добро пожаловать на демонстрацию маршрутизатора!",
            "featured_count": 3}

def projects_loader(params):
    return [{"id": 1, "name": "Flet", "status": "Реализован"},
        {"id": 2, "name": "Flutter", "status": "Реализован"},
        {"id": 3, "name": "FastAPI", "status": "Поддержка"},]

def project_detail_loader(params):
    projects = {
        "1": {"id": 1, "name": "Flet", "description":
            "Создание приложений на Python"},
        "2": {"id": 2, "name": "Flutter", "description":
            "Разработка пользовательского интерфейса от Google"},
        "3": {"id": 3, "name": "FastAPI", "description":
            "Современный веб-фреймворк на Python"},
    }
    return projects.get(params.get("projectId"), {"name": "Unknown",
                                                  "description": ""})


def settings_loader(params):
    return {"sections": ["Профиль", "Безопасность", "Уведомления"]}
...
def LoginPage():
    ...
    return ft.Column(
        [
            ft.Text("Войти в систему", size=28),
            ft.TextField(label="Имя пользователя", value="admin", ref=username_ref),
            ft.Row(
                [ft.Button("Авторизоваться как user", on_click=handle_login),
                 ft.Button("Авторизоваться как admin", on_click=handle_admin_login), ]
            ),
        ],
        width=300, )
    ...
    return ft.Column(
        [
            ...
            ft.Container(
                content=ft.Row(
                    [
                        ft.Image('fl.png',  width=50, height=50),
                        NavLink("Домашняя", "/"),
                        NavLink("Проекты", "/projects"),
                        NavLink("Настройки", "/settings"),
                        ft.Text(f"Вошел: {auth.username}"),
                        ft.IconButton(
                            ft.Icons.LOGOUT,
                            on_click=lambda: (
                                auth.logout(),
...
def Home():
    ...
    return ft.Column(
        [
            ...
            ft.Text(f"Доступны следующие {data['featured_count']} проекта"),
            ft.Button("Показать проекты",
...
def SettingsSection():
    ...
    return ft.Column(
        [ft.Text(f"{params['section'].title()} Settings", size=24),
         ft.Text(f"Настройте {params['section']} в этом разделе."),
         ft.Button("Назад в настройки",
