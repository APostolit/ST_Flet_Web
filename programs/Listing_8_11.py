...
def main(page: ft.Page):
    ...
    message = ft.Text("Анимируйте контейнер!")

    def animate_container(e: ft.Event[ft.Button]):
        message.value = (
            "Вернуть прежнее состояние!"
            if message.value == "Анимируйте контейнер!" else "Анимируйте контейнер!"
        )
            ...
            content=ft.Column(
                controls=[
                    container := ft.Container(
                        width=250, height=250, gradient=gradient2,
                        alignment=ft.Alignment.TOP_LEFT,
                        animate=ft.Animation(
                            duration=1000, curve=ft.AnimationCurve.BOUNCE_OUT),
                        border=ft.Border.all(width=2, color=ft.Colors.BLUE),
                        border_radius=10, padding=10, content=message,),
                    ft.Button("Анимировать контейнер", on_click=animate_container),
                ]
            )
        ),
    )
