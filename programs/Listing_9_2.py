...
@ft.control
class Switcher(ft.AnimatedSwitcher):
...
def main(page: ft.Page):
    page.title = "Анимация изображений"
    switcher = Switcher(
        content=ft.Image(src=f"https://picsum.photos/200/300?{time.time()}",
    ...
    page.add(
        ft.SafeArea(
            content=ft.Column(
                controls=[switcher,
                          ft.Button("Сменить рисунок!", on_click=switcher.animate),]
                )
            ),
        )
