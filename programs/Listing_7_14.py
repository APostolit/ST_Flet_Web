...
def main(page: ft.Page):
    ...
    def image_panel(title: str, image: ft.Image):
        return ft.Column(
            ...
            controls=[ft.Text(title, weight=ft.FontWeight.BOLD),
    ...
    page.appbar = ft.AppBar(title="Демонстрация воспроизведения без задержек")
    page.add(
        ft.SafeArea(
            content=ft.Column(
                controls=[
                    ft.Text("Нажмите 'Загрузить следующее фото'. "
                            "На левом изображении будет отображаться предыдущий кадр "
                            "во время загрузки  следующего кадра. "
                            "Это называется воспроизведением без задержек.",
                            text_align=ft.TextAlign.CENTER,),
                    ft.Row(wrap=True,
                           spacing=20,
                           run_spacing=20,
                           alignment=ft.MainAxisAlignment.CENTER,
                           controls=[image_panel("Загруженное фото",
                                                 image_with_gapless),
                                     image_panel("Загружаемое фото",
                                            image_without_gapless,),
                        ],),
                    ft.FilledButton("Загрузить следующее фото", on_click=next_photo),
                ],
            ),
        )
    )
