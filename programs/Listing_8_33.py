# ResponsiveRow_2
import flet as ft

def main(page: ft.Page):
    page.title = "ResponsiveRow пользовательские точки останова"
    page.padding = 16

    breakpoints = {
        "Смартфон": 0,
        "Планшет": 540,
        "Компьютер": 800,
    }

    sorted_breakpoints = sorted(breakpoints.items(), key=lambda item: item[1])

    breakpoint_labels = {
        name: ft.Text(f"{name}: >= {value}px", weight=ft.FontWeight.W_500)
        for name, value in sorted_breakpoints
    }

    width_label = ft.Text()
    breakpoint_label = ft.Text()

    def update_status(_=None):
        width = (
            (page.window.width if page.window and page.window.width else None)
            or page.width
            or 0
        )
        width_label.value = f"Ширина страницы: {width:.0f}px"
        active_breakpoint = max(
            (bp for bp, min_width in breakpoints.items() if width >= min_width),
            key=lambda bp: breakpoints[bp],
            default="phone",
        )
        breakpoint_label.value = f"Активная точка останова: {active_breakpoint}"
        for name, label in breakpoint_labels.items():
            is_active = name == active_breakpoint
            label.color = ft.Colors.BLUE_700 if is_active else None
            label.weight = ft.FontWeight.W_700 if is_active else ft.FontWeight.W_400
            label.update()
        width_label.update()
        breakpoint_label.update()

    page.on_resize = update_status

    rr = ft.ResponsiveRow(
        breakpoints=breakpoints,
        columns={"Смартфон": 4, "Планшет": 8, "Компьютер": 12},
        spacing=10,
        run_spacing=10,
        controls=[
            ft.Container(content=ft.Text("Карточка 1", size=16,
                                         weight=ft.FontWeight.W_600),
                         alignment=ft.Alignment.CENTER,
                         bgcolor=ft.Colors.AMBER_200,
                         height=60,
                         col={"Смартфон": 4, "Планшет": 4, "Компьютер": 3},),
            ft.Container(content=ft.Text("Карточка 2", size=16,
                                         weight=ft.FontWeight.W_600),
                         alignment=ft.Alignment.CENTER,
                         bgcolor=ft.Colors.GREEN_200,
                         height=60,
                         col={"Смартфон": 4, "Планшет": 4, "Компьютер": 3},),
            ft.Container(content=ft.Text("Карточка 3", size=16,
                                         weight=ft.FontWeight.W_600),
                         alignment=ft.Alignment.CENTER,
                         bgcolor=ft.Colors.BLUE_200,
                         height=60,
                         col={"Смартфон": 4, "Планшет": 4, "Компьютер": 3},),
            ft.Container(content=ft.Text("Карточка 4", size=16,
                                         weight=ft.FontWeight.W_600),
                         alignment=ft.Alignment.CENTER,
                         bgcolor=ft.Colors.PINK_200,
                         height=60,
                         col={"Смартфон": 4, "Планшет": 4, "Компьютер": 3},),
            ],
        )

    page.add(
        ft.SafeArea(
            content=ft.Column(
                controls=[
                    ft.Text("Измените размер окна, чтобы увидеть пользовательские"
                            " точки останова в действии."),
                    ft.Text("Карточки переключают ширину столбцов под телефон, "
                            "планшет компьютер."),
                    ft.Column(
                        [ft.Text("Пользовательские точки останова (минимальная ширина):",
                                 weight=ft.FontWeight.W_600,),
                            ft.Column(list(breakpoint_labels.values()), spacing=2),],
                        spacing=6,),
                    rr,
                    width_label,
                    breakpoint_label,
                ]
            )
        )
    )
    update_status()


if __name__ == "__main__":
    ft.run(main, view=ft.AppView.WEB_BROWSER)