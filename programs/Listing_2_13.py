...
def main(page: ft.Page):
    page.title = "CodeEditor - обработка выбора"
    max_selection_preview = 80

    theme = fce.CustomCodeTheme(
        keyword=ft.TextStyle(color=ft.Colors.YELLOW_ACCENT, weight=ft.FontWeight.W_600),
            ...
            content=ft.Column(
                ...
                controls=[
                    ...
                    ft.Column(
                        margin=10,
                        controls=[
                            selection := ft.Text("Выделите какой-нибудь текст в редакторе."),
                            selection_details := ft.Text(),
                            caret := ft.Text("Положение курсора: -"),
                            ft.Row(
                                controls=[
                                    ft.Button("Выделить весь текст", on_click=select_all),
                                    ft.Button("Переместить курсор на начало",
